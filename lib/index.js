...
/**
 * Every model each lane can serve, in the order the selector should show them.
 *
 * This is the **full catalog**, and it is the single source of truth for three
 * things that must never drift apart:
 *
 *  1. what `cordis.patch.yml` declares in the composition (kept byte-identical to
 *     this table by `scripts/check-catalog.mjs`),
 *  2. what the Models-management page offers as toggleable rows, and
 *  3. what the enable/disable write puts back into
 *     `settings.yaml` → `llm-pi-ai.providers.<route>.models`.
 *
 * Why this table exists at all: a composition patch is a *static* layer. pi-ai's
 * `models` list **replaces** a route's catalog wholesale, so the only way to hide a
 * model from the selector is to omit it — and a user cannot edit a bundle's patch.
 * The supported seam is the user settings layer, which merges over the patch per
 * provider. So "disable a model" means: write the enabled subset into
 * `llm-pi-ai.providers.<route>.models` in `settings.yaml`. That write is live —
 * `llm-pi-ai` re-registers routes from its `onChange` — so toggling needs no restart.
 *
 * Ids and capacities are the ones verified in `docs/verification.md`; nothing here is
 * guessed. `contextWindow` is repeated per entry because a `models` entry replaces the
 * whole entry, so an omitted capacity would fall back to the 262,144 default and
 * silently shrink a 1M-context model.
 */
const CATALOG = {
  'ark-agent-plan': [
    { id: 'ark-code-latest', name: 'Ark 智能路由 (ark-code-latest)', contextWindow: 262144 },
    { id: 'doubao-seed-evolving', name: 'Doubao-Seed-Evolving', contextWindow: 262144 },
    { id: 'doubao-seed-2-1-turbo', name: 'Doubao-Seed-2.1-Turbo', contextWindow: 262144 },
    { id: 'doubao-seed-2-0-lite', name: 'Doubao-Seed-2.0-Lite', contextWindow: 262144 },
    { id: 'doubao-seed-2-0-mini', name: 'Doubao-Seed-2.0-Mini', contextWindow: 262144 },
...
/**
 * The model-management pathname, read and written by the browser half.
 *
 * Same shape and same fencing as {@link QUOTA_ROUTE}: an exact route under
 * `/plugins`, loopback-only. It is a separate path rather than a verb on the quota
 * route because the two answer different questions and only this one writes.
 */
...
/**
 * The settings namespace this plugin owns, where the enabled-model selection lives.
 *
 * It is deliberately NOT the `llm-pi-ai` namespace. That one is owned by the shipped
 * adapter, which validates every write against the whole provider schema; a plugin
 * writing there would be editing another plugin's section and would have to satisfy
 * its `assertServiceable` on every toggle. Instead this plugin records the *intent*
 * (`enabledModelIds`) in its own namespace and applies it to `llm-pi-ai` through the
 * ordinary settings service — the same path a user's hand-edited `settings.yaml` takes.
 */
const SETTINGS_NS = 'dsh-ark-plans'

/**
 * The schema of this plugin's own settings section.
 *
 * `enabledModelIds` is a per-route map of model ids. An **empty list means "all
 * enabled"** — see {@link effectiveEnabled} — so the default posture of a fresh
 * install is the complete catalog, and this section only ever records a narrowing.
 */
const Config = z.object({
  enabledModelIds: z.dict(z.array(z.string())).default({}),
})

/** The route's settings address inside the `llm-pi-ai` namespace. */
const PI_AI_NS = 'llm-pi-ai'

/**
 * Normalize a stored selection against the current catalog.
 *
 * A stored list is intent, not truth: the catalog can grow (a model added in a later
 * version) or shrink (one dropped). So an unknown id is dropped, and — crucially — an
 * **empty or absent** selection means "everything enabled", never "nothing enabled".
 * That is what makes the feature additive: an install that predates it, or a user who
 * never opened the page, keeps the complete catalog rather than silently losing every
 * model to an empty selector.
 *
 * @param route - the lane's route key.
 * @param stored - the raw stored selection, if any.
 * @returns the effective enabled ids, in catalog order.
 */
function effectiveEnabled(route, stored) {
  const catalog = CATALOG[route] ?? []
  if (!Array.isArray(stored) || stored.length === 0) return catalog.map((m) => m.id)
  const known = new Set(catalog.map((m) => m.id))
  const wanted = new Set(stored.filter((id) => typeof id === 'string' && known.has(id)))
  // An all-unknown list is treated as "not configured" rather than "disable all", so a
  // renamed catalog cannot leave the user with an empty picker and no way back.
  if (wanted.size === 0) return catalog.map((m) => m.id)
  return catalog.filter((m) => wanted.has(m.id)).map((m) => m.id)
}

/**
 * Build the `models` array for a lane from a set of enabled ids.
 *
 * The order is always catalog order, so the selector does not reshuffle itself each
 * time a model is toggled — only the membership changes.
 *
 * @param route - the lane's route key.
 * @param enabled - the enabled ids.
 * @returns the `models` entries to write, or the full catalog when nothing is filtered.
 */
function modelsFor(route, enabled) {
  const catalog = CATALOG[route] ?? []
  const on = new Set(enabled)
  return catalog.filter((m) => on.has(m.id)).map((m) => ({ ...m }))
}

/**
 * Read the current selection and the effective catalog for every lane.
 *
 * @param ctx - the plugin's Cordis context.
 * @returns one entry per lane: its full catalog, its enabled ids, and whether the
 *   selection is the user's or the untouched default.
 */
function readSelection(ctx) {
  ...
  if (settings !== undefined) {
    try {
      ...
      if (section !== null && typeof section === 'object' && section.enabledModelIds !== null
        && typeof section.enabledModelIds === 'object') {
        stored = section.enabledModelIds
      }
    } catch {
      // An unreadable section is the default posture, not an error: the page then
      // renders every model as enabled, which is exactly what an absent section means.
      stored = {}
    }
  }
  return PLANS.map((plan) => {
    const catalog = CATALOG[plan.route] ?? []
    const raw = stored[plan.route]
    const enabled = effectiveEnabled(plan.route, raw)
    return {
      route: plan.route,
      plan: plan.label,
      models: catalog.map((m) => ({ id: m.id, name: m.name })),
      enabled,
      configured: Array.isArray(raw) && raw.length > 0,
    }
  })
}

/**
 * Apply one lane's selection to the `llm-pi-ai` section, live.
 *
 * Writes only this route's key, leaving every other provider — including ones the user
 * configured by hand — untouched. `settings.update` merges per namespace, and pi-ai
 * merges per provider, so this cannot clobber a sibling route.
 *
 * The plugin's own namespace records the intent first: if the `llm-pi-ai` write is
 * refused (a catalog validation failure), the intent is still stored, so the page keeps
 * showing what the user chose instead of silently reverting.
 *
 * @param ctx - the plugin's Cordis context.
 * @param route - the lane's route key.
 * @param enabled - the ids to leave enabled.
 * @returns the applied state, for the caller to render.
 */
async function applySelection(ctx, route, enabled) {
  const settings = ctx.get('settings')
  if (settings === undefined) {
    throw new Error('settings service unavailable — cannot persist the model selection')
  }
  const normalized = effectiveEnabled(route, enabled)
  const full = CATALOG[route] ?? []
  // A selection covering the whole catalog is recorded as an empty list in this
  // plugin's own section: that is the "no narrowing" posture, and it keeps tracking the
  // catalog when a later version adds a model. The `llm-pi-ai` write below is a
  // separate matter and always carries the explicit list — see the note there.
  const isFull = normalized.length === full.length
  const record = isFull ? [] : normalized
  ...
  const merged = {
    ...
    enabledModelIds: {
      ...((current !== null && typeof current === 'object'
        && current.enabledModelIds !== null && typeof current.enabledModelIds === 'object')
        ? current.enabledModelIds
        : {}),
      [route]: record,
    },
  }
  await settings.update(SETTINGS_NS, merged)

  // The user layer of `llm-pi-ai`.
  //
  // "Everything enabled" writes the full catalog back, NOT an empty list. An empty
  // `models` list is refused for this route — pi-ai validates that a route the
  // installed catalog does not ship (Ark is hand-declared, so the catalog has nothing
  // for it) resolves at least one model, and answers
  ...
  // So the full list is the only representation that restores the selector. The
  // catalog is the plugin's own and a plugin upgrade rewrites this section from its
  // new table on the next save, so the list cannot go stale in a way that matters.
  const models = modelsFor(route, normalized)
  await settings.update(PI_AI_NS, {
    providers: { [route]: { models } },
  })

  return {
    route,
    models: full.map((m) => ({ id: m.id, name: m.name })),
    enabled: normalized,
    configured: !isFull,
  }
}
...
export function apply(ctx) {
  ...
  // This plugin's own settings section, where the enabled-model selection is stored.
  // Registering it is what makes `settings.update(SETTINGS_NS, …)` legal: the settings
  // service refuses to write an unregistered namespace. `base` is the default posture
  // (nothing narrowed), so an absent section resolves to "all models enabled".
  ctx.inject(['settings'], (settingsCtx) => {
    try {
      settingsCtx.settings.installSection(ctx, SETTINGS_NS, Config, { enabledModelIds: {} }, {
        setSource: () => {},
        onChange: () => {},
      })
    } catch (error) {
      // A registration failure must not take down the model routes this bundle exists
      // to provide; the management page reports the absence instead.
      ctx.logger.error(`${PREFIX} could not register the settings section (${String(error)})`)
    }
  })
  ...
  ctx.inject(['webServer'], (webCtx) => {
    ...
    // The Models-management page's data route. GET returns the catalog plus the
    // current selection; POST applies one lane's selection. Both are loopback-only,
    // and neither reads nor writes a credential.
    //
    // Every failure is answered as a 200 body carrying `ok: false` and a reason the
    // page can render, because a thrown error would only reach the browser as an
    // opaque rejection with nothing to show the user.
    webCtx.effect(() => webCtx.webServer.register({
      kind: 'exact',
      path: MODELS_ROUTE,
      handler: async (req, res) => {
        ...
        try {
          if (req.method === 'GET' || req.method === 'HEAD') {
            writeJson(res, 200, { ok: true, plans: readSelection(ctx) })
            return
          }
          if (req.method !== 'POST') {
            writeJson(res, 405, { ok: false, error: 'method not allowed' })
          ...
          if (!PLANS.some((plan) => plan.route === route)) {
            writeJson(res, 200, { ok: false, error: `unknown route "${route}"` })
            return
          }
          if (!Array.isArray(body.enabled)) {
            writeJson(res, 200, { ok: false, error: 'enabled must be an array of model ids' })
            return
          }
          const applied = await applySelection(ctx, route, body.enabled)
          ctx.logger.info(
            `${PREFIX} ${route}: ${applied.enabled.length}/${applied.models.length} model(s) enabled`,
          )
          writeJson(res, 200, { ok: true, plan: applied, plans: readSelection(ctx) })
        } catch (error) {
          ctx.logger.error(`${PREFIX} model selection write failed — ${String(error)}`)
          writeJson(res, 200, { ok: false, error: String(error) })
        }
      },
    }), `${PREFIX} models route`)
  })
...
/**
 * Read and parse a JSON request body, with a bound.
 *
 * The body is a list of model ids, so it is small by construction; the cap exists so a
 * malformed or hostile request cannot make the host buffer without limit.
 *
 * @param req - the route's request.
 * @returns the parsed body, or `{}` when it is empty or unparseable.
 */
function readJsonBody(req) {
  return new Promise((resolve) => {
    ...
    let settled = false
    const LIMIT = 64 * 1024
    // A promise resolves once; `settle` makes that explicit, because the over-limit path
    // destroys the request and would otherwise also reach the `error`/`end` handlers.
    const settle = (value) => {
      if (settled) return
      settled = true
      resolve(value)
